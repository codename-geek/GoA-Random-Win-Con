--ROM Version
--Last Update: Promise Charm portal-exclusive objective
...
function SeedClearedCheck()
	if SeedCleared then
		return
	end
	if WinConNum == 0 then
		--Get Seed based on 5 random chests
		local seed1 = ReadShort(BAR(Sys3, 0x7, 0xD2), OnPC) -- LoD Throne Room, AP Boost
		local seed2 = ReadShort(BAR(Sys3, 0x7, 0x456), OnPC) -- OC Caverns Entrance, Mythril Shard
		local seed3 = ReadShort(BAR(Sys3, 0x7, 0x3C6), OnPC) -- 100AW Starry Hill, Cosmic Ring
		local seed4 = ReadShort(BAR(Sys3, 0x7, 0x3D2), OnPC) -- 100AW Starry Hill, Style Recipe
		local seed5 = ReadShort(BAR(Sys3, 0x7, 0xAFE), OnPC) -- STT Mansion Basement Corridor, Hi-Potion
		local seedNum = seed1 + seed2 + seed3 + seed4 + seed5
		math.randomseed(seedNum)
		WinConNum = math.random(1,17)
		print("Win con is")
		print(WinConNum)
	end
	...
	if ReadByte(Save+0x36C5)&0x1 == 0x1 then
		ReportCount = ReportCount + 1
	end
	if ReadByte(Save+0x36C5)&0x2 == 0x2 then
		ReportCount = ReportCount + 1
	end
	if ReadByte(Save+0x36C5)&0x4 == 0x4 then
		ReportCount = ReportCount + 1
	end
	if ReadByte(Save+0x36C5)&0x8 == 0x8 then
		ReportCount = ReportCount + 1
	end
	...
	if ReadByte(Save+0x36C6)&0x1 == 0x1 then
		ReportCount = ReportCount + 1
	end
	if ReadByte(Save+0x36C6)&0x2 == 0x2 then
		ReportCount = ReportCount + 1
	end
	if ReadByte(Save+0x36C6)&0x4 == 0x4 then
		ReportCount = ReportCount + 1
	end
	...
	--wincons
	--3 proof
	if WinConNum == 1 and ReadByte(Save+0x36B2) > 0 and ReadByte(Save+0x36B2) > 0 and ReadByte(Save+0x36B2) > 0 then
		SeedCleared = true
	--ABN no Reports
	elseif WinConNum == 2 and magicCount + truePageCount + formCount + summonCount + abilityCount + proofCount + unlockCount >= 63 then
		SeedCleared = true
	--ABN with Reports
	elseif WinConNum == 3 and ReportCount + magicCount + truePageCount + formCount + summonCount + abilityCount + proofCount + unlockCount >= 80 then
		SeedCleared = true
	--75% ABN no Reports
	elseif WinConNum == 4 and magicCount + truePageCount + formCount + summonCount + abilityCount + proofCount + unlockCount >= 47 then
		SeedCleared = true
	--All Magic
	elseif WinConNum == 5 and magicCount == 18 then
		SeedCleared = true
	--Own All Pages
	elseif WinConNum == 6 and truePageCount == 5 then
		SeedCleared = true
	--Own All Forms
	elseif WinConNum == 7 and formCount == 5 then
		SeedCleared = true
	--Own All Summons
	elseif WinConNum == 8 and summonCount == 4 then
		SeedCleared = true
	--Own All Unlocks
	elseif WinConNum == 9 and unlockCount == 26 then
		SeedCleared = true
	--Reach Level 50
	elseif WinConNum == 10 and ReadByte(Save+0x24FF) >= 50 then
		SeedCleared = true
	--All Forms to Level 5
	elseif WinConNum == 11 and ReadByte(Save+0x32F6) >= 5 and ReadByte(Save+0x332E) >= 5 and ReadByte(Save+0x3366) >= 5 and ReadByte(Save+0x339E) >= 5 and ReadByte(Save+0x33D6) >= 5 then
		SeedCleared = true
	--All Forms to Level 6
	...
	elseif WinConNum == 14 and ReadByte(Save+0x363D) >= ObjectiveCount then
		SeedCleared = true
	--75% objectives
	elseif WinConNum == 15 and ReadByte(Save+0x363D) >= (ObjectiveCount * 3 / 4) then
		SeedCleared = true
	--50% objectives
